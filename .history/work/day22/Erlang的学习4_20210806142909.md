...
试着用一个元组表示一座房子,再用一个房子列表表示一条街道,确保能在这些结构中加入数据和取出数据.

```
House={housenum,1}. 
%%添加房子可以用House2={housenum,2}. 
%%获取房子信息也很简单,{X,Y}=House. 
Stress=[House]. 
%%可以用Stress1=[House,House1]. 添加为街道多添加以一个房子.
%%可以用[Z,F]=Stress1. 获取街道里的房子的数据.
```
