...
然后将执行`` Server ! {self(),{get_file,File}} ``但是File为missing所以会返回错误,和没有该文件错误(enoent).
...
添加后的功能,客户端可以给服务端发送文件,然后服务端收到put_file给他发送的File参数,把文件路径赋值给OK然后向客户端发回信息{服务端进程号,Ok赋值后的内容},客户端输出Ok赋值后的内容.
